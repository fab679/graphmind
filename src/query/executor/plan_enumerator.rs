...
use super::logical_plan::{ExpandDirection, LogicalPlanNode, PatternEdge, PatternGraph};
use crate::graph::catalog::GraphCatalog;
use crate::query::ast::{BinaryOp, Expression, WhereClause};
...
mod tests {
    ...
    fn test_enumerate_single_node() {
        ...
        let clause = make_match_clause(vec![make_path("n", vec![Label::new("Person")], vec![])]);
    ...
    fn test_enumerate_asymmetric_picks_cheaper_start() {
        ...
        let clause = make_match_clause(vec![make_path(
            ...
            vec![make_segment(
                None,
                vec![EdgeType::new("WORKS_AT")],
                Direction::Outgoing,
                "c",
                vec![Label::new("Company")],
            )],
        )]);
        let pg = PatternGraph::from_match_clause(&clause);
        let config = EnumerationConfig::default();
    ...
    fn test_enumerate_with_where_clause() {
        ...
        let where_clause = WhereClause {
            predicate: Expression::Binary {
                ...
                op: BinaryOp::Eq,
                right: Box::new(Expression::Literal(PropertyValue::String(
                    "Alice".to_string(),
                ))),
            },
        };
    ...
    fn test_plan_has_expand_into_for_triangle() {
        ...
        pg.nodes.insert(
            "a".to_string(),
            super::super::logical_plan::PatternNode {
        ...
        assert!(
            has_expand_into,
            "Triangle pattern should produce at least one plan with ExpandInto"
        );
    }
    ...
    fn contains_expand_into(plan: &LogicalPlanNode) -> bool {
        match plan {
            ...
            LogicalPlanNode::Join { left, right, .. } => {
                contains_expand_into(left) || contains_expand_into(right)
            }
