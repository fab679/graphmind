...
//!
//! Graphmind supports **multigraphs**: multiple edges can exist between the same pair
...
mod tests {
    ...
    fn test_edge_properties() {
        ...
        assert_eq!(
            edge.get_property("strength").unwrap().as_float(),
            Some(0.95)
        );
        assert_eq!(
            edge.get_property("verified").unwrap().as_boolean(),
            Some(true)
        );
