...
//! Algorithms are implemented in `graphmind-graph-algorithms` crate.
//! This module provides the integration/adapter layer.

use crate::graph::{EdgeType, GraphStore, Label, PropertyValue};
use graphmind_graph_algorithms::{GraphView, NodeId as AlgoNodeId};
use std::collections::HashMap;

// Re-export algorithms
pub use graphmind_graph_algorithms::{
    bfs, bfs_all_shortest_paths, cdlp, count_triangles, dijkstra, edmonds_karp,
    local_clustering_coefficient, local_clustering_coefficient_directed, page_rank, pca, prim_mst,
    strongly_connected_components, weakly_connected_components, CdlpConfig, CdlpResult, FlowResult,
    LccResult, MSTResult, PageRankConfig, PathResult, PcaConfig, PcaResult, PcaSolver, SccResult,
    WccResult,
};
...
pub fn build_view(
    ...
    let nodes: Vec<AlgoNodeId> = if let Some(label_str) = node_label {
        ...
        store.all_nodes().iter().map(|n| n.id.as_u64()).collect()
    };
    ...
    let mut weights = if temp_weights.is_some() {
        Some(Vec::new())
    } else {
        None
    };
