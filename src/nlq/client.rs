...
impl NLQClient {
    ...
    pub async fn generate_cypher(&self, prompt: &str) -> NLQResult<String> {
        match self.config.provider {
            ...
            _ => Err(NLQError::ConfigError(format!(
    ...
    async fn openai_chat(&self, prompt: &str) -> NLQResult<String> {
        ...
        let api_key = self
            ...
            .api_key
            .as_ref()
            .ok_or_else(|| NLQError::ConfigError("OpenAI requires API key".to_string()))?;
        let system_prompt = self
            .config
            .system_prompt
            .clone()
            .unwrap_or_else(|| "You are a Cypher expert.".to_string());
    ...
    async fn gemini_chat(&self, prompt: &str) -> NLQResult<String> {
        ...
        let api_key = self
            ...
            .api_key
            .as_ref()
            .ok_or_else(|| NLQError::ConfigError("Gemini requires API key".to_string()))?;
        let system_prompt = self
            .config
            .system_prompt
            .clone()
...
mod tests {
    ...
    fn test_nlq_client_with_system_prompt() {
        let config = NLQConfig {
            ...
            api_base_url: None,
            system_prompt: Some(
                "You are a graph database expert specialized in medical data.".to_string(),
            ),
        };
        let client = NLQClient::new(&config);
        assert!(client.is_ok());
