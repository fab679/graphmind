...
//! communication, originally designed for Redis. Graphmind implements RESP so that **existing
//! Redis clients** — redis-cli, Python's `redis` library, Node.js `ioredis`, Go's
//! `go-redis` — can connect to Graphmind without any modification. This provides instant
...
//! ## Graph commands
//!
//! Graphmind extends the RESP command set with graph-specific commands:
...
//! protocol, Graphmind avoids the "cold start" problem of needing custom client libraries.
...
pub use resp::{RespError, RespResult, RespValue};
