...
impl CommandHandler {
    ...
    /// Handle a RESP command with tenant store manager (multi-tenant)
    pub async fn handle_command_multi(
        &self,
        value: &RespValue,
        ...
        match cmd_name.as_str() {
            "GRAPH.QUERY" => self.handle_graph_query_multi(args, stores).await,
            "GRAPH.RO_QUERY" => self.handle_graph_ro_query_multi(args, stores).await,
            "GRAPH.DELETE" => self.handle_graph_delete_multi(args, stores).await,
            "GRAPH.LIST" => self.handle_graph_list_multi(args, stores).await,
    ...
    /// Handle a RESP command (single-store backward compat)
    pub async fn handle_command(
        &self,
        value: &RespValue,
        store: &Arc<RwLock<GraphStore>>,
    ) -> RespValue {
        let stores = TenantStoreManager::with_default(Arc::clone(store));
        self.handle_command_multi(value, &stores).await
    }

    /// Handle GRAPH.QUERY command (multi-tenant)
    /// Format: GRAPH.QUERY graph_name "MATCH (n) RETURN n"
    async fn handle_graph_query_multi(
        &self,
        args: &[RespValue],
        stores: &TenantStoreManager,
    ) -> RespValue {
        ...
        // Extract graph name for multi-tenancy
        ...
        debug!("Executing query on graph '{}': {}", graph_name, query_str);

        // Get or create the tenant's store
        let store = stores.get_store(&graph_name).await;
    ...
    /// Handle GRAPH.RO_QUERY (read-only query, multi-tenant)
    async fn handle_graph_ro_query_multi(
        &self,
        args: &[RespValue],
        stores: &TenantStoreManager,
    ) -> RespValue {
        self.handle_graph_query_multi(args, stores).await
    }

    /// Handle GRAPH.DELETE command (multi-tenant)
    async fn handle_graph_delete_multi(
        &self,
        args: &[RespValue],
        stores: &TenantStoreManager,
    ) -> RespValue {
        if args.len() < 2 {
            return RespValue::Error(
                "ERR wrong number of arguments for 'GRAPH.DELETE' command".to_string(),
            );
        }
    ...
    /// Handle GRAPH.LIST command (multi-tenant)
    async fn handle_graph_list_multi(
        &self,
        _args: &[RespValue],
        stores: &TenantStoreManager,
    ) -> RespValue {
        let graphs = stores.list_graphs().await;
        RespValue::Array(
            graphs
                .into_iter()
                .map(|g| RespValue::BulkString(Some(g.into_bytes())))
                .collect(),
        )
    }
...
mod tests {
    ...
    async fn test_ping() {
        ...
        let cmd = RespValue::Array(vec![RespValue::BulkString(Some(b"PING".to_vec()))]);
    ...
    async fn test_graph_list() {
        ...
        let cmd = RespValue::Array(vec![RespValue::BulkString(Some(b"GRAPH.LIST".to_vec()))]);
        let response = handler.handle_command(&cmd, &store).await;
        assert!(matches!(response, RespValue::Array(_)));
    }
    ...
    async fn test_info_command() {
        ...
        let cmd = RespValue::Array(vec![RespValue::BulkString(Some(b"INFO".to_vec()))]);
        let response = handler.handle_command(&cmd, &store).await;
    ...
    async fn test_unknown_command() {
        ...
        let cmd = RespValue::Array(vec![RespValue::BulkString(Some(b"NONEXISTENT".to_vec()))]);
        let response = handler.handle_command(&cmd, &store).await;
    ...
    async fn test_graph_query_wrong_args() {
        ...
        let cmd = RespValue::Array(vec![RespValue::BulkString(Some(b"GRAPH.QUERY".to_vec()))]);
        let response = handler.handle_command(&cmd, &store).await;
        assert_eq!(
            response,
            RespValue::Error("ERR wrong number of arguments for 'GRAPH.QUERY' command".to_string())
    ...
    async fn test_graph_query_null_query() {
        ...
        let response = handler.handle_command(&cmd, &store).await;
        assert_eq!(response, RespValue::Error("ERR null query".to_string()));
    }
    ...
    async fn test_graph_delete_wrong_args() {
        ...
        let cmd = RespValue::Array(vec![RespValue::BulkString(Some(b"GRAPH.DELETE".to_vec()))]);
        let response = handler.handle_command(&cmd, &store).await;
        assert_eq!(
            response,
            RespValue::Error(
    ...
    async fn test_echo_wrong_args() {
        ...
        let cmd = RespValue::Array(vec![RespValue::BulkString(Some(b"ECHO".to_vec()))]);
        let response = handler.handle_command(&cmd, &store).await;
        assert_eq!(
            response,
    ...
    async fn test_ping_with_non_string_message() {
        ...
        assert_eq!(response, RespValue::BulkString(Some(b"PONG".to_vec())));
    }
    ...
    async fn test_format_value_property_boolean() {
        ...
        assert_eq!(result, RespValue::BulkString(Some(b"true".to_vec())));

        let value_false = Value::Property(PropertyValue::Boolean(false));
        let result = handler.format_value(&value_false);
        assert_eq!(result, RespValue::BulkString(Some(b"false".to_vec())));
    }
    ...
    async fn test_format_query_result_multiple_columns_rows() {
        ...
        r1.bind(
            "age".to_string(),
            Value::Property(PropertyValue::Integer(30)),
        );
        batch.push(r1);
        ...
        r2.bind(
            "age".to_string(),
            Value::Property(PropertyValue::Integer(25)),
        );
        batch.push(r2);
    ...
    async fn test_command_with_null_first_element() {
        ...
        let response = handler.handle_command(&cmd, &store).await;
        assert_eq!(response, RespValue::Error("ERR null command".to_string()));
    }
    ...
    async fn test_command_with_non_string_first_element() {
        ...
        let cmd = RespValue::Array(vec![RespValue::Integer(123)]);
        let response = handler.handle_command(&cmd, &store).await;
        match response {
            RespValue::Error(msg) => {
                assert!(msg.contains("ERR"));
    ...
    async fn test_graph_query_with_write_keywords_in_middle() {
        ...
        let cmd = RespValue::Array(vec![
            ...
            RespValue::BulkString(Some(
                b"MATCH (n:Person {name: 'Alice'}) SET n.age = 30 RETURN n".to_vec(),
            )),
        ]);
        ...
        assert!(matches!(
            response,
            RespValue::Array(_) | RespValue::Error(_)
        ));
    }
