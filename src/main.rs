use clap::Parser;
use graphmind::http::HttpServer;
use graphmind::{GraphStore, NodeId, QueryEngine, RespServer, ServerConfig, TenantStoreManager};
...
#[command(
    name = "graphmind",
    version,
    about = "High-performance distributed graph database"
)]
struct Args {
    /// Path to config file (TOML)
    #[arg(short, long, default_value = "graphmind.toml")]
    config: String,
    ...
    /// Run demo on startup (social or large)
    #[arg(long)]
    demo: Option<String>,
}
...
async fn main() {
    ...
    println!("Graphmind Graph Database v{}", graphmind::version());
    println!("==========================================");
    println!();
...
fn build_social_network(store: &mut GraphStore) {
    ...
    let first_names = [
        ...
        "Leo", "Mia", "Noah", "Olivia", "Paul", "Quinn", "Rosa", "Sam", "Tara", "Uma", "Vic",
    ...
    let city_data = [
        ("New York", "US", 8_336_817i64),
        ("Los Angeles", "US", 3_979_576),
        ("Chicago", "US", 2_693_976),
        ("Houston", "US", 2_320_268),
        ("San Francisco", "US", 873_965),
        ("Seattle", "US", 737_015),
        ("Austin", "US", 978_908),
        ("Boston", "US", 675_647),
        ("Denver", "US", 715_522),
        ("Portland", "US", 652_503),
        ("London", "UK", 8_982_000),
        ("Manchester", "UK", 553_230),
        ("Edinburgh", "UK", 524_930),
        ("Berlin", "DE", 3_644_826),
        ("Munich", "DE", 1_471_508),
        ("Hamburg", "DE", 1_841_179),
        ("Paris", "FR", 2_161_000),
        ("Lyon", "FR", 513_275),
        ("Toronto", "CA", 2_794_356),
        ("Vancouver", "CA", 662_248),
        ("Sydney", "AU", 5_312_163),
        ("Melbourne", "AU", 5_078_193),
        ("Tokyo", "JP", 13_960_000),
        ("Singapore", "SG", 5_454_000),
        ("Bangalore", "IN", 8_443_675),
        ("Mumbai", "IN", 12_442_373),
        ("São Paulo", "BR", 12_325_232),
        ("Dublin", "IE", 544_107),
        ("Amsterdam", "NL", 872_680),
        ("Stockholm", "SE", 975_551),
    ];
    let company_data = [
        ("Acme Corp", "Technology", 5000i64),
        ("GlobalBank", "Finance", 12000),
        ("MediCare Plus", "Healthcare", 3500),
        ("EcoEnergy", "Energy", 2200),
        ("DataStream", "Technology", 800),
        ("CloudNine", "Technology", 1500),
        ("BioGenesis", "Healthcare", 4200),
        ("QuantumLeap", "Technology", 350),
        ("GreenField", "Agriculture", 6000),
        ("SkyRoute", "Logistics", 9000),
        ("NexGen AI", "Technology", 600),
        ("FinEdge", "Finance", 2800),
        ("UrbanBuild", "Construction", 7500),
        ("MediaPulse", "Media", 1200),
        ("AeroSpace X", "Aerospace", 4500),
        ("FoodChain", "Retail", 15000),
        ("CyberShield", "Security", 900),
        ("EduPath", "Education", 1800),
        ("TravelWise", "Travel", 3200),
        ("PharmaCore", "Healthcare", 5500),
    ];
    let tags = [
        "rust",
        "python",
    ...
    for i in 0..num_posts {
        ...
        if let Some(node) = store.get_node_mut(id) {
            ...
            node.set_property(
                "content",
                format!(
                    "{}. This is post #{} with detailed thoughts on the topic.",
                    topic, i
                ),
            );
    ...
    for i in 0..num_comments {
        ...
        if let Some(node) = store.get_node_mut(id) {
            node.set_property(
                "text",
                format!("Comment #{}: Great point! Here are my thoughts...", i),
            );
    ...
    println!(
        "  Nodes:  {} Person, {} Company, {} City, {} Tag, {} Post, {} Comment",
        person_ids.len(),
        company_ids.len(),
        city_ids.len(),
        tag_ids.len(),
    ...
    println!(
        "  Edges:  {} KNOWS, {} LIKES, {} Posts, {} Comments (total: {})",
        knows_count,
        likes_count,
        num_posts,
        num_comments,
...
async fn start_server() {
    ...
    // Load config file (optional - don't fail if not found)
    let mut toml_config = if std::path::Path::new(&args.config).exists() {
        let config_str = std::fs::read_to_string(&args.config).unwrap_or_else(|e| {
            eprintln!("Warning: Could not read config file: {}", e);
            String::new()
        });
        toml_crate::from_str::<TomlConfig>(&config_str).unwrap_or_else(|e| {
    ...
    if let Some(host) = args.host {
        toml_config.server.host = host;
    }
    if let Some(port) = args.port {
        toml_config.server.resp_port = port;
    }
    if let Some(http_port) = args.http_port {
        toml_config.server.http_port = http_port;
    }
    if let Some(data_dir) = args.data_dir {
        toml_config.server.data_dir = data_dir;
    }
    if let Some(log_level) = args.log_level {
        toml_config.logging.level = log_level;
    }

    // Env vars override everything
    if let Ok(host) = std::env::var("GRAPHMIND_HOST") {
        toml_config.server.host = host;
    }
    if let Ok(port) = std::env::var("GRAPHMIND_PORT") {
        toml_config.server.resp_port = port.parse().unwrap_or(6379);
    }
    if let Ok(port) = std::env::var("GRAPHMIND_HTTP_PORT") {
        toml_config.server.http_port = port.parse().unwrap_or(8080);
    }
    if let Ok(dir) = std::env::var("GRAPHMIND_DATA_DIR") {
        toml_config.server.data_dir = dir;
    }
    ...
    let persistence = if let Some(path) = &config.data_path {
        match graphmind::PersistenceManager::new(path) {
    ...
    if let Some(ref pm) = persistence {
        match pm.list_persisted_tenants() {
            Ok(tenants) if !tenants.is_empty() => {
                ...
                println!(
                    "Recovery complete. Total: {} nodes, {} edges in-memory",
                    graph.node_count(),
                    graph.edge_count()
                );
            }
    ...
    if !recovered {
        match demo_mode.as_deref() {
            ...
            Some(other) => {
                eprintln!(
                    "Unknown --demo mode '{}'. Use: --demo social  or  --demo large",
                    other
                );
                eprintln!(
                    "  social  — synthetic social network (5K nodes, 6 labels, 9 edge types)"
                );
    ...
    let tenant_stores = TenantStoreManager::with_default(Arc::clone(&store));
    ...
    // Start HTTP server for Visualizer API
    let http_port = toml_config.server.http_port;
    let http_stores = tenant_stores.clone();
    tokio::spawn(async move {
        let http_server = HttpServer::new_multi_tenant(http_stores, http_port);
        println!(
            "HTTP server starting on port {} (visualizer + API)",
            http_port
        );
    ...
    let server = if let Some(pm) = persistence {
        RespServer::new_multi_tenant_with_persistence(config, tenant_stores, pm)
    } else {
        RespServer::new_multi_tenant(config, tenant_stores)
    };
