...
pub fn export_tenant(
    ...
    let header = SnapshotHeader {
        ...
        graphmind_version: crate::VERSION.to_string(),
    };
    ...
    for line_result in lines {
        ...
        if line.contains("\"t\":\"n\"") {
            ...
            let new_src = id_remap.get(&snap_edge.src).ok_or_else(|| {
                format!("edge references unknown source node ID {}", snap_edge.src)
            })?;
            let new_tgt = id_remap.get(&snap_edge.tgt).ok_or_else(|| {
                format!("edge references unknown target node ID {}", snap_edge.tgt)
            })?;
            ...
            if let Some(type_tag) = obj.get("__type").and_then(|v| v.as_str()) {
                match type_tag {
                    ...
                    "Duration" => {
                        ...
                        let nanos = obj.get("nanos").and_then(|v| v.as_i64()).unwrap_or(0) as i32;
...
mod tests {
    ...
    fn test_round_trip_properties() {
        ...
        assert_eq!(node.get_property("int"), Some(&PropertyValue::Integer(42)));
        assert_eq!(
    ...
    fn test_invalid_format_rejected() {
        ...
        let header = SnapshotHeader {
            ...
            graphmind_version: "0.6.0".to_string(),
        };
