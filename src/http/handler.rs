...
/// Handler for multi-statement Cypher scripts
/// Splits input on newlines, skips comments and blanks, executes each statement sequentially.
...
/// Handler for natural language to Cypher translation
/// Uses the NLQ pipeline to convert plain English to Cypher queries.
pub async fn nlq_handler(
    ...
    // Build schema summary from current graph
    let store = state.stores.get_store(&payload.graph).await;
    let store_guard = store.read().await;
    let mut schema_parts = Vec::new();

    // Collect node labels and their properties
    schema_parts.push("Node Labels:".to_string());
    ...
    // Collect edge types from edges
    let mut edge_type_set: BTreeSet<String> = BTreeSet::new();
    for edge in store_guard.all_edges() {
        edge_type_set.insert(edge.edge_type.as_str().to_string());
    }
    schema_parts.push("Relationship Types:".to_string());
    for et in &edge_type_set {
        schema_parts.push(format!("  [:{}]", et));
    }

    let schema_summary = schema_parts.join("\n");
    drop(store_guard);

    // Try to create NLQ pipeline — check environment for API key
    let api_key = std::env::var("OPENAI_API_KEY")
        .ok()
        .or_else(|| std::env::var("GEMINI_API_KEY").ok())
        .or_else(|| std::env::var("ANTHROPIC_API_KEY").ok());

    let (provider, model, key): (LLMProvider, String, Option<String>) = if std::env::var(
        "OPENAI_API_KEY",
    )
    .is_ok()
    {
    ...
    } else if std::env::var("CLAUDE_CODE_NLQ").is_ok() {
        (LLMProvider::ClaudeCode, "claude".to_string(), None)
    } else {
        // No LLM configured — return helpful error
        return Json(json!({
            "error": "NLQ requires an LLM provider. Set one of: OPENAI_API_KEY, GEMINI_API_KEY, or CLAUDE_CODE_NLQ=1",
            "cypher": null,
            "provider": null
        })).into_response();
    };
    ...
    let pipeline = match NLQPipeline::new(config) {
        ...
        Err(e) => {
            return (
                ...
                Json(json!({
                    "error": format!("Failed to initialize NLQ: {}", e),
                    "cypher": null
                })),
            )
                .into_response();
    ...
    match pipeline
        .text_to_cypher(&payload.query, &schema_summary)
        .await
    {
        Ok(cypher) => Json(json!({
            "cypher": cypher,
        ...
        Err(e) => (
            axum::http::StatusCode::BAD_REQUEST,
            Json(json!({
                "error": format!("NLQ translation failed: {}", e),
                "cypher": null
            })),
        )
            .into_response(),
...
pub async fn status_handler(State(state): State<AppState>) -> impl IntoResponse {
    ...
    crate::metrics::update_storage_gauges(node_count as u64, edge_count as u64);
    Json(json!({
        "status": "healthy",
...
pub async fn schema_handler(State(state): State<AppState>) -> impl IntoResponse {
    ...
    for edge_type in store_guard.all_edge_types() {
        ...
        for edge in edges.iter().take(1000) {
            ...
            for (k, v) in &edge.properties {
                edge_props.entry(k.clone()).or_insert_with(|| match v {
        ...
        for (i, node) in nodes.iter().enumerate() {
            ...
            if i % stride == 0 {
                ...
                let name = node
                    ...
                    .iter()
                    .find(|(k, _)| {
                        k.as_str() == "name" || k.as_str() == "title" || k.as_str() == "label"
                    })
...
pub async fn import_csv_handler(
    ...
    let id_col_idx = id_column
        .as_ref()
        .and_then(|id_col| headers.iter().position(|h| h.trim() == id_col.as_str()));
        ...
        Ok(_stats) => (
            ...
            [
                ...
                (
                    ...
                    "attachment; filename=\"snapshot.sgsnap\"",
                ),
            ],
            buf,
        )
...
/// List all graphs/tenants
pub async fn list_graphs_handler(State(state): State<AppState>) -> impl IntoResponse {
    let graphs = state.stores.list_graphs().await;
    Json(json!({ "graphs": graphs }))
}

/// Delete a graph/tenant
pub async fn delete_graph_handler(
    State(state): State<AppState>,
    axum::extract::Path(graph): axum::extract::Path<String>,
) -> impl IntoResponse {
    if state.stores.delete_graph(&graph).await {
        Json(json!({ "status": "deleted", "graph": graph })).into_response()
    } else {
        (
            axum::http::StatusCode::NOT_FOUND,
            Json(json!({ "error": format!("Graph '{}' not found or protected", graph) })),
        )
            .into_response()
    }
}
...
mod tests {
    ...
    async fn test_status_handler_after_data_and_queries() {
        ...
        {
            let store_arc = state.stores.get_store("default").await;
