...
impl NSGA2Solver {
        ...
        for _iter in 0..self.config.max_iterations {
            ...
            while offspring.len() < pop_size {
                ...
                offspring.push(MultiObjectiveIndividual::new(
                    c1_vars.clone(),
                    problem.objectives(&c1_vars),
                ));
                if offspring.len() < pop_size {
                    offspring.push(MultiObjectiveIndividual::new(
                        c2_vars.clone(),
                        problem.objectives(&c2_vars),
                    ));
                }
            }
    ...
    fn evaluate_population(&self, population: &mut [MultiObjectiveIndividual]) {
        ...
        let mut rank = 0;
        loop {
            let current_rank_indices: Vec<usize> = population
                .iter()
                .enumerate()
            ...
            if current_rank_indices.is_empty() {
                break;
            }
    ...
    fn mutate(&self, vars: &mut Array1<f64>, lower: &Array1<f64>, upper: &Array1<f64>) {
        ...
        for i in 0..vars.len() {
            if rng.gen::<f64>() < self.mutation_rate {
                ...
                vars[i] =
                    (vars[i] + (rng.gen::<f64>() - 0.5) * range * 0.1).clamp(lower[i], upper[i]);
            }
        }
    }
}
