...
use graphmind_optimization::common::*;
...
fn test_nsga2_biobjective() {
    ...
    assert!(
        ...
        "NSGA-II should produce a non-empty Pareto front"
    );
    ...
    for ind in &result.pareto_front {
        assert!(
            ind.fitness[0] >= 0.0 && ind.fitness[0] <= 1.0,
            "f1 out of range: {}",
            ind.fitness[0]
        );
        assert!(
            ind.fitness[1] >= 0.0,
            "f2 should be non-negative: {}",
            ind.fitness[1]
        );
    }
}
...
fn test_motlbo_biobjective() {
    ...
    assert!(
        !result.pareto_front.is_empty(),
        "MOTLBO should produce a non-empty Pareto front"
    );
    for ind in &result.pareto_front {
        assert_eq!(ind.rank, 0, "Pareto front members should have rank 0");
    }
...
fn test_result_variables_in_bounds() {
    ...
    for i in 0..problem.dim() {
        assert!(
            result.best_variables[i] >= lower[i] && result.best_variables[i] <= upper[i],
            "Variable {} out of bounds: {} not in [{}, {}]",
            i,
            result.best_variables[i],
            lower[i],
            upper[i]
        );
    }
...
fn test_simple_problem_closure() {
    ...
    assert!(
        result.best_fitness < 0.1,
        "SimpleProblem with DE failed: fitness {}",
        result.best_fitness
    );
...
fn test_de_rastrigin_10d() {
    ...
    assert!(
        ...
        "DE on Rastrigin-10D: fitness {} too high",
        result.best_fitness
    );
}
...
fn test_constrained_problem() {
    ...
    assert!(
        result.best_variables[0] + result.best_variables[1] >= 0.9,
        "Constraint violated: x0+x1 = {}",
        result.best_variables[0] + result.best_variables[1]
    );
}
