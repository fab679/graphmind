...
A high-performance, Rust-native library for graph analytics and topology analysis. Optimized for the Graphmind Graph Database but usable as a standalone crate.
...
```rust
use graphmind_graph_algorithms::common::{GraphView, NodeId};
...
**PageRank**:
```rust
use graphmind_graph_algorithms::pagerank::{page_rank, PageRankConfig};
...
**Shortest Path**:
```rust
use graphmind_graph_algorithms::pathfinding::dijkstra;
...
**Max Flow**:
```rust
use graphmind_graph_algorithms::flow::edmonds_karp;
...
## Integration with Graphmind
This crate is the backend for `algo.*` Cypher procedures in Graphmind.
