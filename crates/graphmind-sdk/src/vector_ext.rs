...
pub trait VectorClient {
    ...
    async fn vector_search(
        ...
        query_vec: &[f32],
        k: usize,
    ) -> GraphmindResult<Vec<(NodeId, f32)>>;
}
...
impl VectorClient for EmbeddedClient {
    async fn create_vector_index(
        ...
        store
            .create_vector_index(label, property, dimensions, metric)
            .map_err(|e| GraphmindError::VectorError(e.to_string()))
    }
...
mod tests {
    ...
    async fn test_vector_index_create_and_search() {
        ...
        // Create index
        client
            .create_vector_index("Doc", "embedding", 4, DistanceMetric::Cosine)
            .await
            .unwrap();

        // Create some nodes
        client
            .query("default", r#"CREATE (d:Doc {title: "Alpha"})"#)
            .await
            .unwrap();
        client
            .query("default", r#"CREATE (d:Doc {title: "Beta"})"#)
            .await
            .unwrap();
        ...
        client
            .add_vector("Doc", "embedding", nodes[0], &[1.0, 0.0, 0.0, 0.0])
            .await
            .unwrap();
        client
            .add_vector("Doc", "embedding", nodes[1], &[0.0, 1.0, 0.0, 0.0])
            .await
            .unwrap();

        // Search
        let results = client
            .vector_search("Doc", "embedding", &[1.0, 0.1, 0.0, 0.0], 2)
            .await
            .unwrap();
