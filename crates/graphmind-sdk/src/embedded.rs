...
use graphmind::query::{QueryEngine, RecordBatch, Value};

use crate::client::GraphmindClient;
use crate::error::{GraphmindError, GraphmindResult};
use crate::models::{QueryResult, SdkEdge, SdkNode, ServerStatus, StorageStats};
...
impl EmbeddedClient {
    ...
    pub fn nlq_pipeline(
        &self,
        config: graphmind::persistence::tenant::NLQConfig,
    ) -> Result<graphmind::NLQPipeline, graphmind::NLQError> {
        graphmind::NLQPipeline::new(config)
    }

    /// Create an agent runtime for agentic enrichment workflows.
    pub fn agent_runtime(
        &self,
        config: graphmind::persistence::tenant::AgentConfig,
    ) -> graphmind::agent::AgentRuntime {
        graphmind::agent::AgentRuntime::new(config)
    }
    ...
    ) -> Result<graphmind::PersistenceManager, graphmind::PersistenceError> {
        graphmind::PersistenceManager::new(base_path)
    }

    /// Return AST cache statistics (hits, misses).
    pub fn cache_stats(&self) -> &graphmind::query::CacheStats {
    ...
    pub async fn export_snapshot(
        ...
        _tenant: &str,
        path: &std::path::Path,
    ) -> Result<graphmind::snapshot::format::ExportStats, Box<dyn std::error::Error>> {
        let store_guard = self.store.read().await;
        let file = std::fs::File::create(path)?;
        let writer = std::io::BufWriter::new(file);
        let stats = graphmind::snapshot::export_tenant(&store_guard, writer)?;
        Ok(stats)
    }
    ...
    pub async fn import_snapshot(
        ...
        _tenant: &str,
        path: &std::path::Path,
    ) -> Result<graphmind::snapshot::format::ImportStats, Box<dyn std::error::Error>> {
        let mut store_guard = self.store.write().await;
        let file = std::fs::File::open(path)?;
        let reader = std::io::BufReader::new(file);
        let stats = graphmind::snapshot::import_tenant(&mut store_guard, reader)?;
        Ok(stats)
    }
}
            ...
            match val {
                ...
                Value::NodeRef(id) => {
                    ...
                    let (labels, properties, node_json) = if let Some(node) = store.get_node(*id) {
                        ...
                        let json =
                            serde_json::json!({ "id": id_str, "labels": [], "properties": {} });
...
impl GraphmindClient for EmbeddedClient {
    async fn query(&self, graph: &str, cypher: &str) -> GraphmindResult<QueryResult> {
        if is_write_query(cypher) {
            ...
            let batch = self
                .engine
                .execute_mut(cypher, &mut *store_guard, graph)
    ...
    async fn delete_graph(&self, _graph: &str) -> GraphmindResult<()> {
        let mut store_guard = self.store.write().await;
        store_guard.clear();
        Ok(())
    }
    ...
    async fn status(&self, _graph: &str) -> GraphmindResult<ServerStatus> {
        ...
        Ok(ServerStatus {
            ...
            version: graphmind::VERSION.to_string(),
    ...
    async fn schema(&self, _graph: &str) -> GraphmindResult<String> {
        ...
        lines.push(format!("Nodes: {}", store_guard.node_count()));
        lines.push(format!("Edges: {}", store_guard.edge_count()));

        // Collect label counts
        let mut label_counts: HashMap<String, usize> = HashMap::new();
        for node in store_guard.all_nodes() {
            for label in &node.labels {
                *label_counts.entry(label.as_str().to_string()).or_insert(0) += 1;
            }
        }
        if !label_counts.is_empty() {
            lines.push("Node labels:".to_string());
        ...
        for edge in store_guard.all_edges() {
            *edge_type_counts
    ...
    async fn explain(&self, _graph: &str, cypher: &str) -> GraphmindResult<QueryResult> {
        ...
        let batch = self
            .engine
            .execute(&prefixed, &*store_guard)
            .map_err(|e| GraphmindError::QueryError(e.to_string()))?;
        Ok(record_batch_to_query_result(&batch, &*store_guard))
    }

    async fn profile(&self, _graph: &str, cypher: &str) -> GraphmindResult<QueryResult> {
...
mod tests {
    ...
    async fn test_embedded_create_and_query() {
        ...
        // Create nodes
        client
            .query("default", r#"CREATE (n:Person {name: "Alice", age: 30})"#)
            .await
            .unwrap();
        client
            .query("default", r#"CREATE (n:Person {name: "Bob", age: 25})"#)
            .await
            .unwrap();

        // Query
        let result = client
            .query_readonly("default", "MATCH (n:Person) RETURN n.name, n.age")
            .await
            .unwrap();
    ...
    async fn test_embedded_with_existing_store() {
        ...
        let result = client
            .query_readonly("default", "MATCH (n:Person) RETURN n.name")
            .await
            .unwrap();
        assert_eq!(result.records.len(), 1);
    }
    ...
    async fn test_embedded_query_returns_nodes() {
        ...
        let result = client
            .query_readonly("default", "MATCH (n:Person) RETURN n")
            .await
            .unwrap();
        assert_eq!(result.records.len(), 1);
    ...
    async fn test_embedded_query_returns_edges() {
        ...
        let result = client
            .query_readonly("default", "MATCH (a)-[r:KNOWS]->(b) RETURN r")
            .await
            .unwrap();
        assert_eq!(result.records.len(), 1);
    ...
    async fn test_embedded_query_returns_null() {
        ...
        let result = client
            .query_readonly("default", "MATCH (n:Person) RETURN n.missing")
            .await
            .unwrap();
        assert_eq!(result.records.len(), 1);
    ...
    async fn test_embedded_multiple_writes_and_reads() {
        ...
        for i in 0..5 {
            client
                .query("default", &format!(r#"CREATE (n:Item {{id: {}}})"#, i))
                .await
                .unwrap();
        }

        let result = client
            .query_readonly("default", "MATCH (n:Item) RETURN n.id")
            .await
            .unwrap();
