...
fn test_optimization_solver_integration() {
    ...
    assert!(
        fitness >= 500.0 && fitness < 505.0,
        "Fitness should be close to 500.0 (Global Opt), got {}",
        fitness
    );

    // 5. Verify Graph Updates
    // Check if properties were actually updated in the store
    let nodes = store.get_nodes_by_label(&graphmind::graph::Label::new("Resource"));
    for node in nodes {
        let allocation = node.get_property("allocation").unwrap().as_float().unwrap();
        assert!(
            allocation >= 4.99,
            "Allocation should be >= 5.0, got {}",
            allocation
        );
