...
fn build_graph() -> (GraphStore, QueryEngine) {
    ...
    for i in 1..=6 {
        engine
            .execute_mut(
                &format!("CREATE (n:Node {{id: {}, name: 'N{}'}})", i, i),
                &mut store,
                "default",
            )
            .unwrap();
    ...
    engine
        .execute_mut(
            "MATCH (a:Node {id:4}),(b:Node {id:5}) CREATE (a)-[:LINK]->(b)",
            &mut store,
            "default",
        )
        .unwrap();
    engine
        .execute_mut(
            "MATCH (a:Node {id:5}),(b:Node {id:1}) CREATE (a)-[:LINK]->(b)",
            &mut store,
            "default",
        )
        .unwrap();
    engine
        .execute_mut(
            "MATCH (a:Node {id:1}),(b:Node {id:4}) CREATE (a)-[:LINK]->(b)",
            &mut store,
            "default",
        )
        .unwrap();
...
fn test_weighted_path() {
    ...
    let r = engine.execute(
        "CALL algo.weightedPath(0, 3, 'weight') YIELD node, cost RETURN node, cost",
        &store,
    );
    match &r {
...
fn test_mst() {
    let (store, engine) = build_graph();
    let r = engine.execute(
        "CALL algo.mst() YIELD source, target, weight RETURN source, target, weight",
        &store,
    );
    match &r {
