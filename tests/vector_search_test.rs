use graphmind::graph::{GraphStore, Label, PropertyMap, PropertyValue};
...
fn test_vector_search_integration() {
    ...
    let results = store
        .vector_search("Person", "embedding", &query, 1)
        .unwrap();
...
fn test_vector_search_update() {
    ...
    store
        .add_label_to_node("default", node_id, "Person")
        .unwrap();

    // Initially not in index (no vector property)
    let results = store
        .vector_search("Person", "embedding", &vec![1.0, 0.0], 1)
        .unwrap();
