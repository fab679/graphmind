use graphmind::agent::{tools::WebSearchTool, AgentRuntime, Tool};
use graphmind::nlq::NLQPipeline;
use graphmind::persistence::tenant::{AgentConfig, LLMProvider, NLQConfig};
use serde_json::json;
use std::sync::Arc;
...
async fn test_agent_runtime_tool_execution() {
    ...
    let args = json!({ "query": "Graphmind Graph" });
