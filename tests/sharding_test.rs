...
use graphmind::raft::{ClusterConfig, ClusterManager};
use graphmind::sharding::{Proxy, Router};
use graphmind::{GraphStore, RespServer, ServerConfig};
use std::sync::Arc;
...
async fn start_node(id: u64, port: u16) -> (Arc<Router>, Arc<ClusterManager>) {
    ...
    let server = RespServer::new(config, store).with_sharding(
        router.clone(),
        proxy,
...
async fn test_request_forwarding() {
    ...
    if !port_available(PORT1).await || !port_available(PORT2).await {
        eprintln!(
            "Skipping sharding test: ports {}/{} not available",
            PORT1, PORT2
        );
        return;
    }
