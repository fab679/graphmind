...
fn test_mvcc_snapshot_isolation() {
    ...
    store
        .set_node_property("default", n1, "balance", 100)
        .unwrap();
    ...
    store
        .set_node_property("default", n1, "balance", 200)
        .unwrap();
    ...
    assert_eq!(
        node_v1.get_property("balance").unwrap().as_integer(),
        Some(100)
    );
    ...
    assert_eq!(
        node_v2.get_property("balance").unwrap().as_integer(),
        Some(200)
    );
    ...
    assert_eq!(
        node_latest.get_property("balance").unwrap().as_integer(),
        Some(200)
    );
}
...
fn test_mvcc_historical_preservation() {
    ...
    for i in 1..=10 {
        ...
        store
            .set_node_property("default", node_id, "v", i as i64)
            .unwrap();
    }
