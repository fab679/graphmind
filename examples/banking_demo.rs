//! Enterprise Banking Demo - Graphmind Graph Database
//!
...
use graphmind_sdk::{
    EmbeddedClient, GraphStore, GraphmindClient, LLMProvider, Label, NLQConfig, NodeId,
    PersistenceManager,
};
        ...
        if let Some(node) = graph.get_node_mut(node_id) {
            for &key in &[
                ...
                if let Some(v) = row.get(key) {
                    node.set_property(key, *v);
                }
            }

            if let Some(v) = row.get("employee_count") {
                if let Ok(n) = v.parse::<i64>() {
                    node.set_property("employee_count", n);
                }
            }
            if let Some(v) = row.get("latitude") {
                if let Ok(n) = v.parse::<f64>() {
                    node.set_property("latitude", n);
                }
            }
            if let Some(v) = row.get("longitude") {
                if let Ok(n) = v.parse::<f64>() {
                    node.set_property("longitude", n);
                }
            }
        }
    ...
    for line_result in lines {
        ...
        if let Some(node) = graph.get_node_mut(node_id) {
            ...
            if let Some(v) = row.get("risk_score") {
                if let Ok(n) = v.parse::<i64>() {
                    node.set_property("risk_score", n);
                }
            }
            if let Some(v) = row.get("annual_income") {
                if let Ok(n) = v.parse::<f64>() {
                    node.set_property("annual_income", n);
                }
            }
            if let Some(v) = row.get("credit_score") {
                if let Ok(n) = v.parse::<i64>() {
                    node.set_property("credit_score", n);
                }
            }
        }
    ...
    for line_result in lines {
        ...
        if let Some(node) = graph.get_node_mut(node_id) {
            ...
            if let Some(v) = row.get("last_transaction_date") {
                if !v.is_empty() {
                    node.set_property("last_transaction_date", *v);
                }
            }
            ...
            for &key in &[
                ...
                if let Some(v) = row.get(key) {
                    if let Ok(n) = v.parse::<f64>() {
                        node.set_property(key, n);
                    }
                }
            }
            if let Some(v) = row.get("term_months") {
                if let Ok(n) = v.parse::<i64>() {
                    node.set_property("term_months", n);
                }
            }
        }
    ...
    for line_result in lines {
        ...
        if let Some(node) = graph.get_node_mut(node_id) {
            ...
            if let Some(v) = row.get("amount") {
                if let Ok(n) = v.parse::<f64>() {
                    node.set_property("amount", n);
                }
            }
            if let Some(v) = row.get("balance_after") {
                if let Ok(n) = v.parse::<f64>() {
                    node.set_property("balance_after", n);
                }
            }
            if let Some(v) = row.get("mcc_code") {
                if let Ok(n) = v.parse::<i64>() {
                    node.set_property("mcc_code", n);
                }
            }
            if let Some(v) = row.get("fraud_score") {
                if let Ok(n) = v.parse::<f64>() {
                    node.set_property("fraud_score", n);
                }
            }
    ...
    for line_result in lines {
        ...
        if let (Some(from_id), Some(to_id)) = (from_id, to_id) {
            if let (Some(from_node), Some(to_node)) =
                (mappings.find(&from_id), mappings.find(&to_id))
            {
...
fn load_relationships(
    ...
    for (filename, edge_type, from_col, to_col) in rel_files.iter() {
        let loaded = load_relationship_file(
            graph, data_dir, mappings, filename, edge_type, from_col, to_col,
        )?;
        if loaded > 0 {
...
fn create_account_transaction_edges(
    ...
    for line_result in lines {
        ...
        if let (Some(&tx_node), Some(&acc_node)) = (
            mappings.transactions.get(tx_id),
            mappings.accounts.get(acc_id),
        ) {
            if graph
                .create_edge(acc_node, tx_node, "HAS_TRANSACTION")
                .is_ok()
            {
                count += 1;
            }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("╔══════════════════════════════════════════════════════════════════════╗");
    println!("║     GRAPHMIND GRAPH DATABASE - Enterprise Banking Demo                 ║");
    println!("╚══════════════════════════════════════════════════════════════════════╝");
    println!();
    ...
    // Persist customers by type to appropriate tenants
    {
        ...
        let individual_customers: Vec<_> = graph
            .get_nodes_by_label(&Label::new("Individual"))
            .into_iter()
        ...
        println!(
            "  ✓ Persisted {} individual customers to retail_banking",
            individual_customers.len()
        );

        let corporate_customers: Vec<_> = graph
            .get_nodes_by_label(&Label::new("Corporate"))
            .into_iter()
            .filter(|n| n.labels.iter().any(|l| l.as_str() == "Customer"))
            .collect();
        ...
        println!(
            "  ✓ Persisted {} corporate customers to corporate_banking",
            corporate_customers.len()
        );

        let hnw_customers: Vec<_> = graph
            .get_nodes_by_label(&Label::new("HighNetWorth"))
            .into_iter()
            .filter(|n| n.labels.iter().any(|l| l.as_str() == "Customer"))
            .collect();
        ...
        println!(
            "  ✓ Persisted {} HNW customers to wealth_management",
            hnw_customers.len()
        );
        ...
        println!(
            "  ✓ Persisted {} edges to corporate_banking",
            corporate_edges
        );
    ...
    let result = client
        .query_readonly("default", "MATCH (c:Customer) RETURN c LIMIT 5")
        .await?;
    println!(
        "  Found {} results ({:.3}ms)",
        result.len(),
    ...
    let q_start = Instant::now();
    let result = client
        .query_readonly("default", "MATCH (c:HighRisk) RETURN c LIMIT 10")
        .await?;
    println!(
        "  Found {} high-risk customers ({:.3}ms)",
        result.len(),
        q_start.elapsed().as_secs_f64() * 1000.0
    );
    ...
    let q_start = Instant::now();
    let result = client
        .query_readonly("default", "MATCH (c:Corporate) RETURN c LIMIT 5")
        .await?;
    println!(
        "  Found {} corporate customers ({:.3}ms)",
        result.len(),
        q_start.elapsed().as_secs_f64() * 1000.0
    );
    ...
    let q_start = Instant::now();
    let result = client
        .query_readonly("default", "MATCH (t:Flagged) RETURN t LIMIT 10")
        .await?;
    println!(
        "  Found {} flagged transactions ({:.3}ms)",
        result.len(),
        q_start.elapsed().as_secs_f64() * 1000.0
    );
    ...
    let q_start = Instant::now();
    let result = client
        .query_readonly("default", "MATCH (b:Branch) RETURN b LIMIT 5")
        .await?;
    println!(
        "  Found {} branches ({:.3}ms)",
        result.len(),
    ...
    let q_start = Instant::now();
    let result = client
        .query_readonly("default", "MATCH (a:Checking) RETURN a LIMIT 5")
        .await?;
    println!(
        "  Found {} checking accounts ({:.3}ms)",
        result.len(),
        q_start.elapsed().as_secs_f64() * 1000.0
    );
    ...
    println!(
        "    High-risk customers (risk_score >= 80): {}",
        high_risk.len()
    );
    ...
    println!(
        "    KNOWS connections from high-risk customers: {}",
        risk_connections
    );
    ...
    println!(
        "    Flagged transactions (fraud_flag = true): {}",
        flagged.len()
    );
    ...
    println!(
        "    Total flagged transaction amount: ${:.2}",
    ...
    println!(
        "    Accounts with Frozen status: {} ({:.1}% of {} accounts)",
        under_review.len(),
        frozen_pct,
        total_accounts
    );
    ...
    if structuring_suspects.is_empty() {
        ...
        println!(
            "      ALERT: {} transactions just under BSA reporting threshold:",
    ...
    if circular_patterns.is_empty() {
        ...
        println!(
            "      ALERT: {} circular transfer patterns detected:",
            circular_patterns.len()
        );
    ...
    for cust_node in &all_customers {
        ...
        let company = cust_node
            .get_property("company_name")
            .and_then(|v| v.as_string())
            .map(|s| s.to_string());

        let cust_id = cust_node
            .get_property("customer_id")
            .and_then(|v| v.as_string())
            .unwrap_or("unknown")
            .to_string();
    ...
    if ofac_matches.is_empty() {
        ...
        println!(
            "  ALERT: {} potential OFAC watchlist matches:",
            ofac_matches.len()
        );
    ...
    if is_claude_available() {
        ...
        let nlq_config = NLQConfig {
            ...
            api_base_url: None,
            system_prompt: Some(
                "You are a Cypher query expert for a banking fraud detection knowledge graph."
                    .to_string(),
            ),
        };
...
fn create_sample_data(graph: &mut GraphStore) -> Result<LoadStats, Box<dyn std::error::Error>> {
    ...
    let customers_data: Vec<(&str, &str, i64, &str, &str, &str, &str, &str)> = vec![
        ...
        (
            "Global Trade Services Corp",
            "Corporate",
            88,
            "Enhanced",
            "compliance@globaltradesvcs.com",
            "Global Trade Services Corp",
            "San Francisco",
            "CA",
        ),
        (
            "Pinnacle Investments Group",
            "Corporate",
            50,
            "Verified",
            "admin@pinnacleinv.com",
            "Pinnacle Investments Group",
            "New York",
            "NY",
        ),
        (
            "Silverline Logistics Inc",
            "Corporate",
            30,
            "Verified",
            "ops@silverlinelogistics.com",
            "Silverline Logistics Inc",
            "Pittsburgh",
            "PA",
        ),
    ...
    for (i, (name, ctype, risk, kyc, email, occupation, city, state)) in
        customers_data.iter().enumerate()
    {
        let node_id = graph.create_node("Customer");
    ...
    println!(
        "    ✓ Created {} sample customers (20 Individual, 5 Corporate, 5 HNW)",
        stats.customers
    );
    ...
    let transactions_data: Vec<(usize, &str, f64, &str, bool)> = vec![
        ...
        (
            ...
            "Transfer",
            15_500.00,
            "Suspicious wire to shell company",
            true,
        ),
        (
            24,
            "Deposit",
            50_000.00,
            "Large cash deposit - unexplained",
            true,
        ),
        ...
        (
            ...
            "Transfer",
            100_000.00,
            "Quarterly investment rebalance",
            false,
        ),
    ...
    for (i, (acc_idx, tx_type, amount, description, flagged)) in
        transactions_data.iter().enumerate()
    {
        let node_id = graph.create_node("Transaction");
    ...
    println!(
        "    ✓ Created {} sample transactions (5 flagged as suspicious)",
        stats.transactions
    );
    ...
    for (acc_idx, &owner_idx) in account_owner.iter().enumerate() {
        if owner_idx < customer_ids.len() && acc_idx < account_ids.len() {
            graph
                .create_edge(customer_ids[owner_idx], account_ids[acc_idx], "OWNS")
                .unwrap();
    ...
    for (i, &cust_id) in customer_ids.iter().enumerate() {
        ...
        graph
            .create_edge(cust_id, branch_ids[branch_idx], "BANKS_AT")
            .unwrap();
        stats.relationships += 1;
    }
    ...
    for (a, b) in &knows_pairs {
        if *a < customer_ids.len() && *b < customer_ids.len() {
            graph
                .create_edge(customer_ids[*a], customer_ids[*b], "KNOWS")
                .unwrap();
            stats.relationships += 1;
        }
    }
    ...
    for (from_idx, to_idx) in &transfer_pairs {
        if *from_idx < account_ids.len() && *to_idx < account_ids.len() {
            graph
                .create_edge(account_ids[*from_idx], account_ids[*to_idx], "TRANSFER_TO")
                .unwrap();
            stats.relationships += 1;
        }
    }
