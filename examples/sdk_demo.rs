//! SDK Demo — demonstrates the graphmind-sdk EmbeddedClient
//!
//! Shows how to use the unified GraphmindClient trait for:
...
use graphmind_sdk::{
    AlgorithmClient, DistanceMetric, EmbeddedClient, GraphStore, GraphmindClient, Label, NodeId,
    PageRankConfig, VectorClient,
};
use std::sync::Arc;
...
#[tokio::main]
async fn main() {
    println!("=== Graphmind SDK Demo (EmbeddedClient) ===\n");
    ...
    let scores = client
        .page_rank(PageRankConfig::default(), Some("Person"), None)
        .await;
    ...
    let wcc = client
        .weakly_connected_components(Some("Person"), None)
        .await;
    ...
    client
        .create_vector_index("Person", "embedding", 3, DistanceMetric::Cosine)
        .await
        .unwrap();
    ...
    ];
    {
        ...
        for (i, emb) in embeddings.iter().enumerate() {
            if i < nodes.len() {
                client
                    .add_vector("Person", "embedding", nodes[i], emb)
                    .await
                    .unwrap();
            }
        }
    ...
    let results = client
        .vector_search("Person", "embedding", &[0.95, 0.05, 0.0], 3)
        .await
        .unwrap();
