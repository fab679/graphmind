//! Drug Interactions & Pharmacogenomics KG Loader — Graphmind Graph Database
//!
...
async fn main() -> Result<(), Error> {
    ...
    let data_dir = if let Some(pos) = args.iter().position(|a| a == "--data-dir") {
        ...
        eprintln!(
            "Usage: cargo run --release --example druginteractions_loader -- --data-dir <PATH>"
        );
        eprintln!();
        eprintln!("Options:");
        eprintln!(
            "  --data-dir PATH   Directory with drugbank/, dgidb/, sider/ subdirs (required)"
        );
    ...
    eprintln!("Drug Interactions KG Loader — Graphmind Graph Database");
    eprintln!();
