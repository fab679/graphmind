...
//!
//! Demonstrates Graphmind's Raft-based HA features:
...
use graphmind::{
    ClusterConfig, ClusterManager, GraphStateMachine, PersistenceManager, RaftNode, RaftRequest,
};
use std::sync::Arc;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("╔══════════════════════════════════════════════════════════════════╗");
    println!("║   GRAPHMIND High Availability Cluster Demo                        ║");
    println!("╚══════════════════════════════════════════════════════════════════╝");
    println!();
    ...
    let mut config = ClusterConfig::new("graphmind-production".to_string(), 3);
    ...
    let peers = vec![
        graphmind::RaftNodeIdWithAddr::new(1, "10.0.1.100:5000".to_string()),
        graphmind::RaftNodeIdWithAddr::new(2, "10.0.2.100:5000".to_string()),
        graphmind::RaftNodeIdWithAddr::new(3, "10.0.3.100:5000".to_string()),
    ];
    ...
    manager
        .update_node_role(1, graphmind::raft::cluster::NodeRole::Leader)
        .await;
    manager
        .update_node_role(2, graphmind::raft::cluster::NodeRole::Follower)
        .await;
    manager
        .update_node_role(3, graphmind::raft::cluster::NodeRole::Follower)
        .await;
    ...
    println!(
        "  Voters:             {}/{}",
        health.active_voters, health.total_voters
    );
    println!(
        "  Has Leader:         {}",
    ...
    println!(
        "  Quorum Satisfied:   {}",
        if health.active_voters > health.total_voters / 2 {
            "Yes"
        } else {
            "No"
        }
    ...
    for (i, (label, name, region, status)) in entities.iter().enumerate() {
        let request = RaftRequest::CreateNode {
            ...
            properties: {
                let mut props = graphmind::PropertyMap::new();
                props.insert(
                    "name".to_string(),
                    graphmind::PropertyValue::String(name.to_string()),
                );
                props.insert(
                    "region".to_string(),
                    graphmind::PropertyValue::String(region.to_string()),
                );
                props.insert(
                    "status".to_string(),
                    graphmind::PropertyValue::String(status.to_string()),
                );
                props
            },
        };
    ...
    println!(
        "    Quorum:     {} (need {})",
        health.active_voters,
        health.total_voters / 2 + 1
    );
    println!(
        "    Can write:  {}",
        if health.active_voters > health.total_voters / 2 {
    ...
    let partition_write = RaftRequest::CreateNode {
        ...
        properties: {
            ...
            props.insert(
                "name".to_string(),
                graphmind::PropertyValue::String("partition-alert".to_string()),
            );
            props.insert(
                "severity".to_string(),
                graphmind::PropertyValue::String("Warning".to_string()),
            );
            props
        },
    };
    ...
    println!(
        "  Write during partition: {:?} (quorum maintained)",
        write_result
    );
    println!();
    ...
    manager
        .add_node(4, "10.0.4.100:5000".to_string(), false)
        .await?;
    ...
    for &node_id in &upgrade_order {
        ...
        if node_id == 1 {
            ...
            manager
                .update_node_role(2, graphmind::raft::cluster::NodeRole::Leader)
                .await;
            manager
                .update_node_role(1, graphmind::raft::cluster::NodeRole::Follower)
                .await;
    ...
    println!(
        "  Leader:  {}",
        if health.has_leader { "Present" } else { "None" }
    );
