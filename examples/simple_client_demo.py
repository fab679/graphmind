"""
Graphmind Graph Database - Python RESP Client Demo

Demonstrates connecting to the Graphmind RESP server and running queries.
Requires a running Graphmind server: cargo run -- --port 6379
...
class GraphmindClient:
    """Simple RESP client for Graphmind Graph Database."""
...
def connect_with_retry(host, port, max_retries=3, delay=2.0):
    ...
    client = GraphmindClient(host, port)
...
def main():
    print("=" * 60)
    print("  Graphmind Graph Database - Python RESP Client Demo")
    print("=" * 60)
    ...
    print("[1] Connecting to Graphmind...")
    try:
    ...
    queries = [
        ("CREATE (n:Company {name: 'Graphmind AI', founded: 2024, domain: 'Graph Database'})",
         "Create company node"),
        ...
        ("CREATE (n:Product {name: 'GraphmindDB', version: '0.5.0', language: 'Rust'})",
         "Create product node"),
    ]
