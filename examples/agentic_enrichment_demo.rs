...
use graphmind_sdk::{AgentConfig, EmbeddedClient, GraphmindClient, LLMProvider, NLQConfig};
use std::collections::HashMap;

#[tokio::main]
async fn main() {
    println!("================================================================");
    println!("  Graphmind Agentic Enrichment Demo");
    ...
    let agent_config = AgentConfig {
        ...
        api_base_url: None,
        system_prompt: Some(
            "You are a pharmaceutical knowledge graph builder. Generate Cypher CREATE statements."
                .to_string(),
        ),
        tools: vec![],
        policies,
    ...
    let cypher_query = match nlq_pipeline
        .text_to_cypher(user_query, schema_summary)
        .await
    {
        Ok(cypher) => {
        ...
        Err(e) => {
            println!(
                "  NLQ translation failed: {} — falling back to default query",
                e
            );
    ...
    println!(
        "Parsed {} Cypher statements from response.",
