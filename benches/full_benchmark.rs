//! Comprehensive Benchmark Suite for Graphmind Graph Database
//!
...
use graphmind::algo::{bfs, build_view, page_rank, weakly_connected_components, PageRankConfig};
use graphmind::persistence::TenantManager;
use graphmind::{DistanceMetric, GraphStore, Label, PropertyValue, QueryEngine};
use rand::Rng;
use std::sync::Arc;
...
async fn main() {
    ...
    println!("╔══════════════════════════════════════════════════════════════════╗");
    println!("║   GRAPHMIND Comprehensive Benchmark Suite                         ║");
    println!("╚══════════════════════════════════════════════════════════════════╝");
    println!();
    ...
    println!(
        "  - Edges:      ~{:>9}",
        format_number(NUM_NODES * EDGES_PER_NODE)
    );
...
async fn benchmark_ingestion(store: &mut GraphStore) -> Vec<graphmind::graph::NodeId> {
    println!("┌──────────────────────────────────────────────────────────────────┐");
    println!("│ Benchmark 1: Data Ingestion                                     │");
    println!("└──────────────────────────────────────────────────────────────────┘");

    store
        .create_vector_index("Entity", "embedding", VECTOR_DIM, DistanceMetric::Cosine)
        .unwrap();
    ...
    println!(
        "    Throughput: {:>10.0} nodes/sec",
        NUM_NODES as f64 / node_time.as_secs_f64()
    );
    println!();
...
fn benchmark_vector_search(store: &GraphStore) {
    ...
    for &k in &k_values {
        ...
        for _ in 0..num_searches {
            ...
            let t = Instant::now();
            let _ = store
                .vector_search("Entity", "embedding", &query, k)
                .unwrap();
...
fn benchmark_k_hop(store: &GraphStore, engine: &QueryEngine) {
    ...
    let queries = [
        (
            "1-hop",
            "MATCH (a:Entity)-[:LINKS_TO]->(b:Entity) WHERE a.id = {} RETURN b.id",
        ),
        (
            "2-hop",
            "MATCH (a:Entity)-[:LINKS_TO]->(b)-[:LINKS_TO]->(c:Entity) WHERE a.id = {} RETURN c.id",
        ),
    ];

    println!(
        "  {:>8} {:>12} {:>14} {:>12}",
        "Hops", "Avg Latency", "QPS", "Queries"
    );
    println!(
        "  {:>8} {:>12} {:>14} {:>12}",
        "----", "-----------", "---", "-------"
    ...
    for _ in 0..num_queries {
        ...
        let mut frontier = vec![graphmind::graph::NodeId::new((start_node + 1) as u64)];
    ...
    println!(
        "  {:>8} {:>10.2?} {:>12.0}/s {:>12}",
        "3-hop",
        hop3_time / num_queries as u32,
        num_queries as f64 / hop3_time.as_secs_f64(),
        num_queries
    );
...
fn benchmark_graph_algorithms(store: &GraphStore) {
    ...
    println!(
        ...
        "WCC",
        wcc_time,
        format!("{} components", wcc.components.len())
    );
    ...
    if view.node_count > 1 {
        ...
        println!(
            ...
            "BFS",
            bfs_time,
            format!("found={}, depth={}", found, depth)
        );
    }
    ...
    println!(
        ...
        "PageRank x10",
        multi_pr_time,
        format!("{:.2?}/iter", multi_pr_time / iterations as u32)
    );
...
fn benchmark_mixed_workload(store: &mut GraphStore, engine: &QueryEngine) {
    ...
    println!(
        "  {:>8} {:>10} {:>12} {:>14}",
        "Type", "Count", "Total Time", "Avg Latency"
    );
    println!(
        "  {:>8} {:>10} {:>12} {:>14}",
        "----", "-----", "----------", "-----------"
