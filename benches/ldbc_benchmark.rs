//! LDBC SNB Interactive Query Benchmark — Graphmind Graph Database
//!
//! Benchmarks Graphmind's query engine against all 21 LDBC SNB Interactive workload queries
...
/// Build the list of 21 LDBC SNB Interactive queries adapted for Graphmind.
///
...
async fn run_benchmark(client: &EmbeddedClient, query: &LdbcQuery, runs: usize) -> BenchResult {
...
async fn main() -> Result<(), Error> {
    ...
    let filter_query: Option<String> = if let Some(pos) = args.iter().position(|a| a == "--query") {
        Some(
            args.get(pos + 1)
                .expect("--query requires a query ID (e.g. IS1, IC3)")
                .to_uppercase(),
        )
    } else {
        None
    ...
    // Load dataset
    // ========================================================================
    eprintln!("LDBC SNB Interactive Benchmark — Graphmind v0.5.8");
    eprintln!();
    ...
    if queries.is_empty() {
        ...
        eprintln!(
            "Available: IS1-IS7, IC1-IC14, INS1-INS8 (with --updates), DEL1-DEL8 (with --deletes)"
        );
        std::process::exit(1);
    }
