//! LDBC FinBench Query Benchmark — Graphmind Graph Database
//!
//! Benchmarks Graphmind's query engine against 40+ LDBC FinBench workload queries
...
fn read_writes() -> Vec<FinBenchQuery> {
    vec![
        ...
        FinBenchQuery {
            ...
            cypher:
                "MATCH (p:Person {id: 2}) SET p.isBlocked = true RETURN p.id, p.name, p.isBlocked",
        },
    ]
}
...
async fn run_benchmark(client: &EmbeddedClient, query: &FinBenchQuery, runs: usize) -> BenchResult {
...
async fn main() -> Result<(), Error> {
    ...
    let filter_query: Option<String> = if let Some(pos) = args.iter().position(|a| a == "--query") {
        Some(
            args.get(pos + 1)
                .expect("--query requires a query ID (e.g. CR-1, SR-3)")
                .to_uppercase(),
        )
    } else {
        None
    ...
    // Load dataset
    // ========================================================================
    eprintln!("LDBC FinBench Benchmark — Graphmind v0.5.8");
    eprintln!();
