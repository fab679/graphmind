...
fn main() {
    ...
    println!();
    println!(
        "  Nodes: {}  |  Edges/node: {}  |  Queries: {}",
        NUM_NODES, EDGES_PER_NODE, NUM_QUERIES
    );
    println!();
    ...
    for i in 0..NUM_NODES {
        ...
        store
            ...
            .unwrap();
        store
            .set_node_property("default", id, "name", format!("node_{}", i))
            .unwrap();
        store
            .set_node_property("default", id, "score", (i as f64) * 0.1)
            .unwrap();
    }
    for i in 0..NUM_NODES {
        let source = graphmind::graph::NodeId::new((i + 1) as u64);
        for _ in 0..EDGES_PER_NODE {
            let target_idx = rng.gen_range(0..NUM_NODES);
            let target = graphmind::graph::NodeId::new((target_idx + 1) as u64);
            if source != target {
    ...
    println!("└──────────────────────────────────────────────────────────────────┘");
    {
        ...
        for _ in 0..NUM_QUERIES {
            let start_node = graphmind::graph::NodeId::new(rng.gen_range(1..=NUM_NODES as u64));
        ...
        println!(
            "    Avg: {:>10.2?}  |  QPS: {:>8.0}  |  Avg reached: {:.0}",
            avg,
            qps,
            total_reached as f64 / NUM_QUERIES as f64
        );
    }
    println!();
