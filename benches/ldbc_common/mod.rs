...
use graphmind_sdk::{GraphStore, NodeId, PropertyValue};
...
pub fn props_place(headers: &[&str], fields: &[&str]) -> Vec<(&'static str, PropertyValue)> {
    ...
    if let Some(v) = field_str(headers, fields, "url") {
        props.push(("url", PropertyValue::String(v)));
    }
    if let Some(v) = field_str(headers, fields, "type") {
        props.push(("type", PropertyValue::String(v)));
    }
    props
}
...
pub fn props_person(headers: &[&str], fields: &[&str]) -> Vec<(&'static str, PropertyValue)> {
    let mut props = Vec::new();
    if let Some(v) = field_str(headers, fields, "firstName") {
        props.push(("firstName", PropertyValue::String(v)));
    }
    if let Some(v) = field_str(headers, fields, "lastName") {
        props.push(("lastName", PropertyValue::String(v)));
    }
    if let Some(v) = field_str(headers, fields, "gender") {
        props.push(("gender", PropertyValue::String(v)));
    }
    if let Some(v) = field_i64(headers, fields, "birthday") {
        props.push(("birthday", PropertyValue::DateTime(v)));
    }
    if let Some(v) = field_i64(headers, fields, "creationDate") {
        props.push(("creationDate", PropertyValue::DateTime(v)));
    }
    if let Some(v) = field_str(headers, fields, "locationIP") {
        props.push(("locationIP", PropertyValue::String(v)));
    }
    if let Some(v) = field_str(headers, fields, "browserUsed") {
        props.push(("browserUsed", PropertyValue::String(v)));
    }
    props
}
...
pub fn props_forum(headers: &[&str], fields: &[&str]) -> Vec<(&'static str, PropertyValue)> {
    let mut props = Vec::new();
    if let Some(v) = field_str(headers, fields, "title") {
        props.push(("title", PropertyValue::String(v)));
    }
    if let Some(v) = field_i64(headers, fields, "creationDate") {
        props.push(("creationDate", PropertyValue::DateTime(v)));
    }
...
pub fn props_post(headers: &[&str], fields: &[&str]) -> Vec<(&'static str, PropertyValue)> {
    let mut props = Vec::new();
    if let Some(v) = field_str(headers, fields, "imageFile") {
        props.push(("imageFile", PropertyValue::String(v)));
    }
    if let Some(v) = field_i64(headers, fields, "creationDate") {
        props.push(("creationDate", PropertyValue::DateTime(v)));
    }
    ...
    if let Some(v) = field_str(headers, fields, "language") {
        props.push(("language", PropertyValue::String(v)));
    }
    if let Some(v) = field_str(headers, fields, "content") {
        props.push(("content", PropertyValue::String(v)));
    }
    if let Some(v) = field_i64(headers, fields, "length") {
        props.push(("length", PropertyValue::Integer(v)));
    }
    props
}
...
pub fn load_multi_value_attrs(
    ...
    if email_path.exists() {
            ...
            if let Some(&node_id) = person_ids.get(pid) {
                if let Some(node) = graph.get_node_mut(node_id) {
                    if emails.len() == 1 {
                        ...
                        let arr: Vec<PropertyValue> = emails
                            .iter()
        ...
        eprintln!(
            "  Enriched {} persons with email addresses",
            format_num(email_map.len())
        );
    }
    ...
    if lang_path.exists() {
            ...
            if let Some(&node_id) = person_ids.get(pid) {
                if let Some(node) = graph.get_node_mut(node_id) {
                    if langs.len() == 1 {
                        ...
                        let arr: Vec<PropertyValue> = langs
                            .iter()
        ...
        eprintln!(
            "  Enriched {} persons with languages",
            format_num(lang_map.len())
        );
    }
...
pub fn load_dataset(graph: &mut GraphStore, data_dir: &Path) -> Result<LoadResult, Error> {
    ...
    let n = load_nodes(
        ...
        "Organisation",
        graph,
        &mut ids.organisation,
        props_organisation,
    )?;
    print_done(&format!(
        "  Organisation:  {:>12} nodes ({})",
        format_num(n),
        format_duration(t.elapsed())
    ));
    total_nodes += n;
    ...
    let t = std::time::Instant::now();
    let n = load_nodes(
        &dynamic_dir.join("person_0_0.csv"),
        "Person",
        graph,
        &mut ids.person,
        props_person,
    ...
    let t = std::time::Instant::now();
    let n = load_nodes(
        &dynamic_dir.join("forum_0_0.csv"),
        "Forum",
        graph,
        &mut ids.forum,
        props_forum,
    ...
    let t = std::time::Instant::now();
    let n = load_nodes(
        &dynamic_dir.join("comment_0_0.csv"),
        "Comment",
        graph,
        &mut ids.comment,
        props_comment,
    ...
    let t = std::time::Instant::now();
    let n = load_edges(
        &static_dir.join("place_isPartOf_place_0_0.csv"),
        "IS_PART_OF",
        graph,
        &ids.place,
        &ids.place,
    ...
    print_done(&format!(
        "  IS_PART_OF (Place->Place):             {:>12} edges ({})",
        format_num(n),
        format_duration(t.elapsed())
    ));
    total_edges += n;
    ...
    let t = std::time::Instant::now();
    let n = load_edges(
        &static_dir.join("organisation_isLocatedIn_place_0_0.csv"),
        "IS_LOCATED_IN",
        graph,
        &ids.organisation,
        &ids.place,
    ...
    print_done(&format!(
        "  IS_LOCATED_IN (Org->Place):            {:>12} edges ({})",
        format_num(n),
        format_duration(t.elapsed())
    ));
    total_edges += n;
    ...
    let t = std::time::Instant::now();
    let n = load_edges(
        &static_dir.join("tag_hasType_tagclass_0_0.csv"),
        "HAS_TYPE",
        graph,
        &ids.tag,
        &ids.tagclass,
    ...
    print_done(&format!(
        "  HAS_TYPE (Tag->TagClass):              {:>12} edges ({})",
        format_num(n),
        format_duration(t.elapsed())
    ));
    total_edges += n;
    ...
    let t = std::time::Instant::now();
    let n = load_edges(
        &static_dir.join("tagclass_isSubclassOf_tagclass_0_0.csv"),
        "IS_SUBCLASS_OF",
        graph,
        &ids.tagclass,
        &ids.tagclass,
    ...
    print_done(&format!(
        "  IS_SUBCLASS_OF (TagClass->TagClass):   {:>12} edges ({})",
        format_num(n),
        format_duration(t.elapsed())
    ));
    total_edges += n;
    ...
    let t = std::time::Instant::now();
    let n = load_edges(
        &dynamic_dir.join("person_isLocatedIn_place_0_0.csv"),
        "IS_LOCATED_IN",
        graph,
        &ids.person,
        &ids.place,
    ...
    print_done(&format!(
        "  IS_LOCATED_IN (Person->Place):         {:>12} edges ({})",
        format_num(n),
        format_duration(t.elapsed())
    ));
    total_edges += n;
    ...
    let t = std::time::Instant::now();
    let n = load_edges(
        &dynamic_dir.join("person_knows_person_0_0.csv"),
        "KNOWS",
        graph,
        &ids.person,
        &ids.person,
    ...
    let t = std::time::Instant::now();
    let n = load_edges(
        &dynamic_dir.join("person_hasInterest_tag_0_0.csv"),
        "HAS_INTEREST",
        graph,
        &ids.person,
        &ids.tag,
    ...
    print_done(&format!(
        "  HAS_INTEREST (Person->Tag):            {:>12} edges ({})",
        format_num(n),
        format_duration(t.elapsed())
    ));
    total_edges += n;
    ...
    let t = std::time::Instant::now();
    let n = load_edges(
        &dynamic_dir.join("person_studyAt_organisation_0_0.csv"),
        "STUDY_AT",
        graph,
        &ids.person,
        &ids.organisation,
    ...
    let t = std::time::Instant::now();
    let n = load_edges(
        &dynamic_dir.join("person_workAt_organisation_0_0.csv"),
        "WORK_AT",
        graph,
        &ids.person,
        &ids.organisation,
    ...
    let t = std::time::Instant::now();
    let n = load_edges(
        &dynamic_dir.join("person_likes_post_0_0.csv"),
        "LIKES",
        graph,
        &ids.person,
        &ids.post,
    ...
    let t = std::time::Instant::now();
    let n = load_edges(
        &dynamic_dir.join("person_likes_comment_0_0.csv"),
        "LIKES",
        graph,
        &ids.person,
        &ids.comment,
    ...
    let t = std::time::Instant::now();
    let n = load_edges(
        &dynamic_dir.join("forum_hasModerator_person_0_0.csv"),
        "HAS_MODERATOR",
        graph,
        &ids.forum,
        &ids.person,
    ...
    print_done(&format!(
        "  HAS_MODERATOR (Forum->Person):         {:>12} edges ({})",
        format_num(n),
        format_duration(t.elapsed())
    ));
    total_edges += n;
    ...
    let t = std::time::Instant::now();
    let n = load_edges(
        &dynamic_dir.join("forum_hasMember_person_0_0.csv"),
        "HAS_MEMBER",
        graph,
        &ids.forum,
        &ids.person,
    ...
    print_done(&format!(
        "  HAS_MEMBER (Forum->Person):            {:>12} edges ({})",
        format_num(n),
        format_duration(t.elapsed())
    ));
    total_edges += n;
    ...
    let t = std::time::Instant::now();
    let n = load_edges(
        &dynamic_dir.join("forum_hasTag_tag_0_0.csv"),
        "HAS_TAG",
        graph,
        &ids.forum,
        &ids.tag,
    ...
    let t = std::time::Instant::now();
    let n = load_edges(
        &dynamic_dir.join("forum_containerOf_post_0_0.csv"),
        "CONTAINER_OF",
        graph,
        &ids.forum,
        &ids.post,
    ...
    print_done(&format!(
        "  CONTAINER_OF (Forum->Post):            {:>12} edges ({})",
        format_num(n),
        format_duration(t.elapsed())
    ));
    total_edges += n;
    ...
    let t = std::time::Instant::now();
    let n = load_edges(
        &dynamic_dir.join("post_hasCreator_person_0_0.csv"),
        "HAS_CREATOR",
        graph,
        &ids.post,
        &ids.person,
    ...
    print_done(&format!(
        "  HAS_CREATOR (Post->Person):            {:>12} edges ({})",
        format_num(n),
        format_duration(t.elapsed())
    ));
    total_edges += n;
    ...
    let t = std::time::Instant::now();
    let n = load_edges(
        &dynamic_dir.join("post_hasTag_tag_0_0.csv"),
        "HAS_TAG",
        graph,
        &ids.post,
        &ids.tag,
    ...
    let t = std::time::Instant::now();
    let n = load_edges(
        &dynamic_dir.join("post_isLocatedIn_place_0_0.csv"),
        "IS_LOCATED_IN",
        graph,
        &ids.post,
        &ids.place,
    ...
    print_done(&format!(
        "  IS_LOCATED_IN (Post->Place):           {:>12} edges ({})",
        format_num(n),
        format_duration(t.elapsed())
    ));
    total_edges += n;
    ...
    let t = std::time::Instant::now();
    let n = load_edges(
        &dynamic_dir.join("comment_hasCreator_person_0_0.csv"),
        "HAS_CREATOR",
        graph,
        &ids.comment,
        &ids.person,
    ...
    print_done(&format!(
        "  HAS_CREATOR (Comment->Person):         {:>12} edges ({})",
        format_num(n),
        format_duration(t.elapsed())
    ));
    total_edges += n;
    ...
    let t = std::time::Instant::now();
    let n = load_edges(
        &dynamic_dir.join("comment_hasTag_tag_0_0.csv"),
        "HAS_TAG",
        graph,
        &ids.comment,
        &ids.tag,
    ...
    let t = std::time::Instant::now();
    let n = load_edges(
        &dynamic_dir.join("comment_isLocatedIn_place_0_0.csv"),
        "IS_LOCATED_IN",
        graph,
        &ids.comment,
        &ids.place,
    ...
    print_done(&format!(
        "  IS_LOCATED_IN (Comment->Place):        {:>12} edges ({})",
        format_num(n),
        format_duration(t.elapsed())
    ));
    total_edges += n;
    ...
    let t = std::time::Instant::now();
    let n = load_edges(
        &dynamic_dir.join("comment_replyOf_comment_0_0.csv"),
        "REPLY_OF",
        graph,
        &ids.comment,
        &ids.comment,
    ...
    print_done(&format!(
        "  REPLY_OF (Comment->Comment):           {:>12} edges ({})",
        format_num(n),
        format_duration(t.elapsed())
    ));
    total_edges += n;
    ...
    let t = std::time::Instant::now();
    let n = load_edges(
        &dynamic_dir.join("comment_replyOf_post_0_0.csv"),
        "REPLY_OF",
        graph,
        &ids.comment,
        &ids.post,
    ...
    print_done(&format!(
        "  REPLY_OF (Comment->Post):              {:>12} edges ({})",
        format_num(n),
        format_duration(t.elapsed())
    ));
    total_edges += n;
