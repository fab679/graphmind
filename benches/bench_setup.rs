...
//!   1. `GRAPHMIND_LICENSE_KEY` env var (raw token)
//!   2. `GRAPHMIND_LICENSE_FILE` env var (path to file)
//!   3. `./graphmind.license` default file
//!
...
fn init_enterprise() {
    ...
    match license_token {
        Some(token) => {
            match graphmind::license::License::from_token(&token) {
                Ok(lic) => {
                    if lic.is_valid() {
                        ...
                        if lic.feature_enabled("gpu") {
                            graphmind_gpu::GpuContext::enable_licensed();
                            if graphmind_gpu::GpuContext::is_available() {
                            ...
                            println!(
                                "[bench] GPU feature not included in license. Running CPU only."
                            );
                        }
                    } else {
                ...
                Err(e) => {
                    println!(
                        "[bench] Invalid license: {}. Running in Community mode (CPU only).",
                        e
                    );
                }
            }
...
fn resolve_license_token() -> Option<String> {
    // 1. GRAPHMIND_LICENSE_KEY env var (raw token)
    if let Ok(key) = std::env::var("GRAPHMIND_LICENSE_KEY") {
        if !key.is_empty() {
            return Some(key);
        }
    }
    ...
    // 2. GRAPHMIND_LICENSE_FILE env var (path to file)
    if let Ok(path) = std::env::var("GRAPHMIND_LICENSE_FILE") {
    ...
    // 3. Default file: ./graphmind.license
    if let Ok(contents) = std::fs::read_to_string("graphmind.license") {
        let token = contents.trim().to_string();
        if !token.is_empty() {
            return Some(token);
        }
