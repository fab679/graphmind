...
This document outlines the strategic plan to transition Graphmind from a monolithic structure to a modular ecosystem of specialized crates. This approach improves compilation times, enforces cleaner boundaries, and enables independent usage of components.

## 1. Modularization Strategy (Refactor)

The first phase involves extracting existing core components into standalone crates.

### `crates/graphmind-vector` (Vector Search Engine)
...
### `crates/graphmind-raft` (Consensus Module)
...
### `crates/graphmind-cypher` (Query Compiler)
...
### `crates/graphmind-graph-algorithms` (Graph Algorithms) ✅ **Exists**
*   **Current State**: Already extracted as a standalone crate in `crates/graphmind-graph-algorithms/`.
*   **Implemented Algorithms** (8): PageRank, BFS, Dijkstra, WCC, SCC, MaxFlow (Edmonds-Karp), MST (Prim's), TriangleCount.

### `crates/graphmind-optimization` (Optimization Solvers) ✅ **Exists**
*   **Current State**: Already extracted as a standalone crate in `crates/graphmind-optimization/`.
...
### `crates/graphmind-ml` (Predictive Inference)
...
### `crates/graphmind-geo` (Spatial Indexing)
*   **Concept**: First-class support for geospatial data types and operations.
*   **Rationale**: Critical for logistics, supply chain, and routing use cases which are primary targets for Graphmind.
*   **Planned Features**:
...
1.  ~~**Phase 0 (Done)**: Extract `graphmind-graph-algorithms` and `graphmind-optimization` to `crates/`.~~ ✅
2.  **Phase 1 (Refactor)**: Extract `graphmind-vector` and `graphmind-raft` to `crates/`.
3.  **Phase 2 (New Feature)**: Build `graphmind-ml` prototype with ONNX support.
4.  **Phase 3 (Utility)**: Extract `graphmind-cypher` and implement `graphmind-geo` as required by user demand.
