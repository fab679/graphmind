# GNN Implementation Strategy for Graphmind Graph

## Recommendation: Implement GNN Inference, Not Training

Graphmind Graph should implement GNN capabilities by focusing strictly on **Inference (Execution)** rather than **Training**.
...
As an AI-native Graph Vector Database, Graphmind already provides:
...
Most graph databases act as passive stores. By enabling in-database inference, Graphmind becomes a dynamic **Feature Store**, capable of updating node properties (e.g., `n.risk_score`) in real-time as the graph evolves.
