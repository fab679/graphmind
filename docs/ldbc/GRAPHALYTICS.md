...
|-----------|-------------|-------------|----------------|
| Breadth-First Search | BFS | Single-source shortest path (unweighted) | `graphmind_graph_algorithms::bfs()` |
| PageRank | PR | Iterative link analysis ranking | `graphmind_graph_algorithms::page_rank()` |
| Weakly Connected Components | WCC | Find connected components (ignoring edge direction) | `graphmind_graph_algorithms::weakly_connected_components()` |
| Community Detection (Label Propagation) | CDLP | Assign community labels via neighbor voting | `graphmind_graph_algorithms::cdlp()` |
| Local Clustering Coefficient | LCC | Measure of neighborhood connectivity per node | `graphmind_graph_algorithms::local_clustering_coefficient_directed()` |
| Single-Source Shortest Path | SSSP | Weighted shortest path from source | `graphmind_graph_algorithms::dijkstra()` |
