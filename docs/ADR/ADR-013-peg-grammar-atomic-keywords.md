...
Graphmind uses the [Pest](https://pest.rs/) PEG (Parsing Expression Grammar) parser for OpenCypher query parsing. The grammar is defined in `src/query/cypher.pest` and covers keywords like `MATCH`, `WHERE`, `RETURN`, `AND`, `OR`, `ORDER BY`, `NOT`, `IN`, `CONTAINS`, `STARTS WITH`, `ENDS WITH`, and others.
